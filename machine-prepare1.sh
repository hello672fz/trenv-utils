...
function generate_cp() {
  ...
  cat /var/lib/faasd/secrets/basic-auth-password | faas-cli login -u admin --password-stdin -g http://127.0.0.1:8081 
  ...
  faasd checkpoint --dax-device \$DAX_DEVICE --mem-pool \$POOL_TYPE h-hello-world
}
